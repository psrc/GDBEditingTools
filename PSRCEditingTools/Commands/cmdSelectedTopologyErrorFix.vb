...
<ComClass(cmdSelectedTopologyErrorFix.ClassId, cmdSelectedTopologyErrorFix.InterfaceId, cmdSelectedTopologyErrorFix.EventsId), _
 ProgId("PSRCEditingTools.cmdSelectedTopologyErrorFix")> _
Public NotInheritable Class cmdSelectedTopologyErrorFix
...
#Region "COM GUIDs"
    ...
    Public Const ClassId As String = "0fd2b09a-122d-44aa-9bf8-25af626bddf2"
    Public Const InterfaceId As String = "ebbd971a-28a7-4c7b-a316-3c4acbdaa99a"
    Public Const EventsId As String = "d7f00b88-4ba0-47ac-a47c-077c7495b1e5"
#End Region
...
#End Region
    ...
    Public Sub New()
        ...
        MyBase.m_caption = "Fixes Selected Topology Error"   'localizable text 
        MyBase.m_message = "Fixes Selected Topology Error"   'localizable text 
        MyBase.m_toolTip = "Fixes Selected Topology Error" 'localizable text 
        MyBase.m_name = "PSRCEditingTools.cmdSelectedTopologyErrorFix" 'unique id, non-localizable (e.g. "MyCategory_ArcMapCommand")
    ...
    Private Sub Topologyfix3()
        ...
        If topologyExt.ActiveErrorCount = 0 Then
            MessageBox.Show("No topology error selected. Please select one from the error inspector and try again!", "No Topology Error Selected!", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            'Dim topoError As ITopologyErrorFeature
            topoError = topologyExt.ActiveError(0)
            If topoError.TopologyRuleType = esriTopologyRuleType.esriTRTLineEndpointCoveredByPoint Then
                ...
                If MessageBox.Show("A junction needs to added. Are you sure you want to add one?", "Junction Needed", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    'pFeature = topoError
                    ...
                    topologyExt.RemoveActiveError(topoError, ESRI.ArcGIS.EditorExt.esriTEEventHint.esriTEFixedActiveErrors)
            ...
            If topoError.TopologyRuleType = esriTopologyRuleType.esriTRTLineCoveredByLineClass Then
                ...
                'topoRuleContainter.PromoteToRuleException(topoError)
                'Else
                If MessageBox.Show("A Reference Edge needs to added. Are you sure you want to add one?", "Reference Edge Needed", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
        ...
        If topoError.TopologyRuleType = esriTopologyRuleType.esriTRTPointCoveredByLineEndpoint Then
            If MessageBox.Show("A Reference Edge may need to be split. Make sure that the Junction in question is covered by an existing edge. Are you sure you want to split this edge?", "Edge Needs to be Split", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                pFeature = topoError
                refEdgeCollection = SplitEdgeTopologyFix3(m_RefEdges, m_Junctions, topoError, m_application, m_cmdStartEditing.pWrkspc)
            ...
            End If
        End If
        'pTopologyExtension.TopologySelectionChanged  pTopologyExtension.DelayEvents False
        topologyExt.TopologySelectionChanged()
        topologyExt.DelayEvents(False)

        topologyExt.ClearActiveErrors(ESRI.ArcGIS.EditorExt.esriTEEventHint.esriTEValidate)
        topologyExt.ClearActiveErrors(ESRI.ArcGIS.EditorExt.esriTEEventHint.esriTEFixedActiveErrors)
        topologyExt.RemoveActiveError(topoError, ESRI.ArcGIS.EditorExt.esriTEEventHint.esriTEFixedActiveErrors)
